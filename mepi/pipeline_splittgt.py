...
def run(cfg):
    ...
    if central_freq < 2: os.system(f"{mask_ms_command} --mask {rfimask} --accumulation_mode or --memory 4096 --uvrange 0~1000 --statistics {ms_tgt_file}")
    lib_mepi.print_flags(ms_tgt_file)
    casa.flagmanager(vis=ms_tgt_file, mode='save', versionname='PreAoflagger')
    os.system(f"{aoflagger_command} -strategy {aoflagger_strategy1} -column CORRECTED_DATA {ms_tgt_file} >> aoflagger.log")
    os.system(f"{aoflagger_command} -strategy {aoflagger_strategy1} -column CORRECTED_DATA {ms_tgt_file} >> aoflagger.log") # twice
    lib_mepi.print_flags(ms_tgt_file)
    ...
    for cc in range(10):
        ...
        os.system(f'{wsclean_command} -name IMG/{Targets}-selfcal-c{cc}  -update-model-required -pol I \
            ...
            {ms_tgtavg_file} > wsclean_{Targets}-selfcal.log')
        
        if cc == 1 or cc == 3:
            log.info(f'Flagging data for self-calibration cycle {cc+1}')
            os.system(f"{shadems_command} -x FREQ -y CORRECTED_DATA:amp --corr XX,YY --png './PLOTS/Tgt-c{cc}.png' {ms_tgtavg_file} >> shadems.log")
            ...
            os.system(f"{shadems_command} -x FREQ -y CORRECTED_DATA:amp --corr XX,YY --png './PLOTS/Tgt-c{cc}-flag.png' {ms_tgtavg_file} >> shadems.log")
